...
int main(int argc, char* argv[]) {
 ...
 // If you want to exctract some file or files.. use this!!!! :)
  // Love you bro -> shkudw -> this is the new shit!!
    ...
    for (auto const& [diskNum, targets] : targets_by_disk) {
        ...
        for (int i = 5; i < MAX_ENTRIES && !remaining_targets.empty(); i++) {
            if (!ReadMFTEntryUsingRunlist(hDisk, (ULONGLONG)i, recBuf.data(), g_MFTEntrySize)) continue;
    ...
    printf("\n==========================================================\n");
    printf("[+] All tasks complete. Exiting.\n");
    return 0;
}
